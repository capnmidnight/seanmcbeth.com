...
function create5(group, value2, ...extensions) {
  return new MediaType(group, value2, extensions);
}
function specialize(group) {
  return create5.bind(null, group);
}
...
var BaseVideoPlayer = class extends BaseAudioSource {
  constructor(type2, context, spatializer) {
    ...
    const audio = BaseVideoPlayer.createMediaElement(Audio2, controls(false));
...
(async function() {
  ...
  elementApply(videosSelect, ...Array.from(videos.keys()).map(
    (title) => Option(
      title,
      value(title)
    )
